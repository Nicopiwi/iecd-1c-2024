...
```{r ejercicio8}
#plot(NULL, xlim = range(Perim_encef), ylim = range(Talla), xlab = "Perimetro encefalico", ylab = "Talla")

bags <- c()
subset_data <- c()
for (sexo in unique(Sexo)) {
  subset_data[[sexo]] <- datos_encuesta[Sexo == sexo, ]
  bags[[sexo]] <- compute.bagplot(subset_data[sexo])
}

ggplot(datos_encuesta, aes(Perim_encef, Talla, color = Sexo)) +
  geom_polygon(data = bags["Mujer"]$hull.loop, fill = "green") +
  #geom_polygon(data = bags["Mujer"]$hull.bag, fill = "green") +
  #geom_point(data = bags["Mujer"]$pxy.outlier, col = "green", pch = 16) +
  geom_polygon(data = bags["Varon"]$hull.loop, fill = "red") +
  #geom_polygon(data = bags["Varon"]$hull.bag, fill = "red") +
  #geom_point(data = bags["Varon"]$pxy.outlier, col = "red", pch = 16) +
  labs(title = "Bagplot by Category", x = "Perimetro encefalico", y = "Talla Variable") +
